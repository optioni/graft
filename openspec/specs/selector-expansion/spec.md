# Selector Expansion Specification

## Purpose

A consumer's `graft.toml` names the items it wants as selectors; a source's
`catalog.yaml` says what it provides. This capability is the meeting of the two: it turns
a source's selector list into the items it names, with globbing in the name position and
an error — never a warning, never a silent empty result — for a selector that matches
nothing. It is pure: a catalog, a source name, and a selector list in, items or an error
out. It reads no file and runs no command.
