...
`catalog.yaml` is the source repository's offer: which kinds of thing it publishes,
where each kind belongs, and which items it provides. It is human-authored in the source
repo and is the least trusted of graft's three files — a human on the other side of a git
URL writes it, and graft acts on it. This capability covers reading and validating it:
every rule that can be decided from the file alone, with no manifest, no lock, no git,
and no network. It reads; it never writes.

graft holds no vocabulary of kinds and never will, so validation here is structural. A
kind is well-formed or it is not; it is never *recognised*.
