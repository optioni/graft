...
`graft sync` is the command that makes a consumer's tree match `graft.lock`. This capability
covers the order it does things in, what it refuses, and what it never does.

The order is the contract rather than an implementation detail. Nothing is written until the
plan is built, so a failure at any earlier step leaves the tree exactly as it was; and the pin
check runs before the first fetch, so a manifest that moved cannot cause network access, let
alone a write.

What sync never does is re-resolve a pin. It installs what the lock says, whatever the rev
names today, so `rev = "main"` cannot drift between syncs. Moving a pin is `graft update`,
always explicit — which is why there is no `--force` and no `--frozen`.
