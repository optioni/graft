...
`graft update` is the one command that moves a pin. Everything else in graft is built on the
promise that pins do not move on their own — `graft sync` installs what `graft.lock` says, so
`rev = "main"` cannot drift between syncs — and a promise like that needs somewhere for the
movement to be explicit. This capability is that place.

An update is a sync with one answer changed: a source's sha comes from re-resolving its rev
rather than from the lock. The sequence, the fetch, the plan, the applier, the prune rule, the
lock written last, and the report are the same code, because the order they happen in is the
contract and two copies of it means two places for it to drift. What this capability adds on
top is `--to`, which writes `graft.toml` — the only path in `update` that does.
