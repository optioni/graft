...
A fetched entry is a directory of files. `internal/plan` cannot look at one — it is pure —
so something must turn a tree into the values a plan is built from: the source's
`catalog.yaml`, and, per item, the `plan.Listing` its `from` contributes. That is this
capability, and it lives beside the fetch because this is the only place that knows where a
fetched tree is.

It reads; it never writes. Containment is the load-bearing part: a source commits the tree
being read, so it may commit a symlink anywhere in it, and an ordinary path operation
follows one straight out of the entry. SPEC.md's invariant is that every file an item reads
stays inside that item's own `from`, and it is enforced here by resolving every path within
the entry rather than by inspecting strings.
