# Destination Computation Specification

## Purpose

A source's catalog says where each kind of thing belongs; a consumer's manifest may
overrule it. This capability turns those declarations plus one item's file listing into
repo-relative destinations — `{name}` interpolation, the trailing-slash rule, `flatten`, a
list-valued `to`, and the per-source override — and says when that mapping is refused.

It is pure. Every input is a value: a parsed catalog, a parsed manifest source, and a
listing of what an item's `from` contributes. Nothing here reads a file, stats a path, runs
a command, or writes anything — enumerating a fetched tree belongs to the package that
fetched it, and writing belongs to `internal/apply`.

Two of its rules exist because a source repository is not trusted. A destination may not
escape the repo root, and a listing entry may not climb out of its own item — in either
direction, since the same entry names both where a file lands and which file is read.
