# Fetch Cache Specification

## Purpose

A resolved sha names an immutable tree, so the tree it names can be stored once and reused
forever. `~/.cache/graft/<host>/<owner>/<repo>/<sha>/` is that store, and its immutability
is what makes SPEC.md's "network unavailable, cache hit: proceeds" true: a sync against a
lock whose shas are already cached runs no git command at all.

The corollary is the whole of the rest of this capability. Nothing will ever re-fetch an
entry that exists, so an incomplete entry would be wrong forever — which is why an entry is
built beside its final path and published by renaming, and why every failure leaves the
cache exactly as it found it.

A fetched tree is untrusted input, and the cache is where that stops being an abstraction:
a remote controls the path segments an entry is derived from, a commit controls the
`.gitattributes` a checkout would honour, and graft may itself be running inside a git
operation whose environment points at the consumer's repository. Each is a rule below.
