...
- [x] 3.1 RED: Add failing cases to `TestParse_VersionErrors` for `A version literal wider than any integer type says to upgrade` (`version: 99999999999999999999999999`, asserting the literal appears in the message as written) and `A hugely negative version literal is not a known version`
- [x] 3.2 RED: Add the cases that bound the rule — `version: true`, `version: "99999999999999999999999999x"`, and the quoted `version: "-1"` under `A version that is neither an integer nor an integer literal is an error`, plus `A sign or separators do not change the answer` (`+99…` and the quoted wide literal, both reporting the upgrade message) — and confirm the existing `version: "1"` and `version: 1.5` cases are present and asserting `catalog.yaml: version must be an integer`, because they are what stops the shape rule over-reaching
- [x] 3.3 Confirmed: all five wide cases (bare, separated, signed, quoted, and hugely negative) returned `catalog.yaml: version must be an integer`, while the three bounding cases (`true`, the trailing-letter string, and the quoted `"-1"`) were already green — the decoder hands back a `string` and the type switch falls through
- [x] 3.4 GREEN: Add a `string` case to `checkVersion` that reads the literal's shape — optional `+`/`-`, decimal digits with optional `_` separators, and a value that **overflows** every 64-bit integer type — reporting the existing upgrade message for a positive literal and the existing not-a-known-version message for a negative one, printing the literal exactly as written. Strip `_` before the range test and count only `strconv.ErrRange`: `ParseUint` fails on `99…9_0` with `ErrSyntax`, and conflating the two failures routes that literal back to the non-integer message the case exists to avoid
- [x] 3.5 GREEN: Leave every other string on `version must be an integer`, and keep the existing `uint64`, `int64`, and `int` branches untouched — a literal that fits never arrives as a string, which is the whole reason the shape test is exact
- [x] 3.6 REFACTOR: done — `wideLiteral` sits directly below `checkVersion` and its comment says outright that it is not a general number parser, names the one input class it serves, and records why `ErrSyntax` and `ErrRange` are not treated alike
- [x] 3.7 CHECK (confirmed: the rule is honoured for every integer literal now — `9223372036854775808` through the `uint64` branch, and anything wider through the shape branch, both printed as written; nothing is half-read, since no catalog is returned on any of them): Contract gate — re-read ENGINEERING.md → Compatibility ("a graft that meets a format version it does not know fails and says to upgrade — it never guesses, and never half-reads a newer file") and confirm the routing now honours it for every integer literal, whatever its width
- [x] 3.8 Run `go test -race ./internal/catalog/...` — green, no regressions

## 4. Destinations are compared as paths, not as strings
<!-- kind: behavior -->

- [x] 4.1 RED: Add failing cases to `TestParse_KindErrors` for `Two spellings of one destination are an error naming both` (`[".claude/agents/", ".claude/agents//"]`) and `A dot-slash prefix is the same destination` (`["a/b", "./a/b"]`), each asserting the exact `duplicate destination "<b>": same path as "<a>"` text, and confirm the existing identical-spelling case still asserts the unchanged one-string message
- [x] 4.2 GUARD (expected green, before and after): Write `TestParse_KindTrailingSlashIsNotADuplicate` for `A trailing slash makes two destinations, not one` (`["docs/{name}", "docs/{name}/"]`), asserting both entries survive in declared order — this is the case design.md → D7 narrows the finding around, and without it the obvious repair breaks a live `destination-computation` requirement
- [x] 4.3 GUARD (expected green, before and after): Add the case for `An uncleaned destination is carried verbatim` (`to: "./.claude//agents/"`) to `TestParse_Kinds`, asserting `To[0]` byte for byte, so "the comparison cleans, the value does not" is asserted rather than reviewed
- [x] 4.4 Confirmed: both new cases returned a parsed catalog carrying both spellings and a nil error, while `TestParse_KindTrailingSlashIsNotADuplicate` and `TestParse_KindUncleanedDestinationIsVerbatim` were green before the change as well as after. Confirm 4.1's cases fail because both spellings survive the raw-string comparison, and confirm 4.2 and 4.3 pass already — they are regression guards for behavior that must *not* change, which is why they are labelled GUARD rather than RED
- [x] 4.5 GREEN: Key the duplicate check in `destinations()` on `path.Clean(d)` with a trailing `/` re-appended when `d` ended in one, keep `To` verbatim, and emit the two-spelling message only when the two entries differ as strings — an identical pair keeps the existing message
- [x] 4.6 CHECK (confirmed: both named tests pass, and the whole `internal/plan` suite is green — the parse-time rule collapses only spellings that mean one path under either reading, so the per-item rule still owns the trailing-slash decision): Concentration point — run `go test -race ./internal/plan/...` and confirm `TestDestination_TrailingSlashAliasesAreOneDestination` and `TestDestination_TrailingSlashAliasesAreTwoDestinationsForAFileItem` are both green. `internal/plan` owns the per-item rule this comparison must not pre-empt, and those two tests are the boundary between them
- [x] 4.7 REFACTOR: done — the key is built by `destKey`, named for the identity it computes rather than for cleaning, with the trailing-slash reason and the `a/b/.` limitation in its comment. `parseKinds`' own comment now says where the single cleaned form in the package lives and that it is never stored
- [x] 4.8 CHECK (confirmed: SPEC.md's bullets make `to` a destination or a list of them with `{name}` and a trailing slash both meaningful, and `destination-computation`'s "same destination when they *mean* the same destination" is the same rule read per item — parse decides the spellings that agree under either reading, plan decides the one that depends on whether `from` is a file): Contract gate — re-read SPEC.md's `kinds.<kind>.to` bullets and the `destination-computation` requirement "Two entries SHALL be the same destination when they *mean* the same destination", and confirm the parse-time rule and the plan-time rule agree rather than overlap
- [x] 4.9 Run `go test -race ./...` — green across all nine packages, no regressions anywhere in the module

## 5. Documentation
<!-- kind: operational -->

- [x] 5.1 CHECK (confirmed: the file has a Phase 1 table, a dependency graph, and a "Notes on the ordering" section, and mentioned `catalog-hardening` nowhere): Re-read `openspec/IMPLEMENTATION-ORDER.md` and confirm this change is absent from it, and that the file's own instruction ("This is a plan, not a commitment … Update this file when that happens rather than letting it drift") makes recording it the expected action rather than an optional tidy-up
- [x] 5.2 CHANGE: done — a Phase 1 row depending on `catalog-and-selectors`, two edges in the mermaid graph (`catalog-and-selectors --> catalog-hardening --> sync-command`), and a note saying why it is not optional before `sync-command`: two of the four findings report success while under-installing. Add `catalog-hardening` to Phase 1 with one line of what it delivers and a dependency on `catalog-and-selectors`, and note in "Notes on the ordering" that it is a precondition of `sync-command` because that is where `Load` first becomes reachable. Document: `openspec/IMPLEMENTATION-ORDER.md`; audience: every future planning session; durable reason: a follow-up change that exists only in an archived tasks.md is invisible to the roadmap that is meant to be read first
