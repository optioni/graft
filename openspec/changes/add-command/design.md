...
| A git value with no usable last segment is refused | Table test over the derivation function, plus the acceptance run that never starts git | unit | none — pure over strings | `go test ./internal/add/` |
...
| A new selector joins an existing source | Direct `add.Run` against a fixture source repo | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
| A selector already declared is not written twice | Direct `add.Run` against a fixture source repo | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
| The same selector given twice is written once | Direct `add.Run` against a fixture source repo | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
| A different repository under a taken name is refused | Direct `add.Run` against a fixture source repo | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
...
| Adding a selector to a branch pin does not move it | Direct `add.Run` against a fixture source repo | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
...
| A catalog is listed with its destinations | `add.List`'s lines against a fixture source repo, with the repository asserted empty afterwards | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
| A consumer override is reflected in the listing | `add.List`'s lines against a fixture source repo, with the repository asserted empty afterwards | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
| A source offering no items lists none | `add.List`'s lines against a fixture source repo, with the repository asserted empty afterwards | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
| An ungraftable source is refused under --list | `add.List`'s lines against a fixture source repo, with the repository asserted empty afterwards | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
...
| Adding a source reports one line | `add.Run`'s report lines against a fixture source repo, plus the acceptance test for the stream they land on | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
| Moving a pin and adding a selector reports both, in order | `add.Run`'s report lines against a fixture source repo, plus the acceptance test for the stream they land on | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
| An invocation that changes nothing says so | `add.Run`'s report lines against a fixture source repo, plus the acceptance test for the stream they land on | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
...
| An unamendable manifest is refused in the amender's words | Direct `add.Run` against a fixture source repo | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
| An unparsable graft.toml is refused before anything is resolved | Direct `add.Run` against a fixture source repo | integration | real filesystem, real `git`, fixture remote, cache root in `t.TempDir()` | `go test ./internal/add/` |
