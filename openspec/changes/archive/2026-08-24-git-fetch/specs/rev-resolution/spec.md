...
`internal/source` SHALL resolve a source's `rev` — a tag, a branch, or a full sha — to the
40-character lowercase hex commit sha that `graft.lock` records as `resolved`. Resolution
SHALL use `git ls-remote` against the source's clone URL, querying `refs/tags/<rev>`,
`refs/tags/<rev>^{}`, and `refs/heads/<rev>` explicitly rather than by pattern, so a rev
can never match a ref whose name merely ends with it.
...
Resolution SHALL fail, returning an empty sha, when no queried ref matches, when the remote
cannot be reached, when `rev` is empty, or when `git` is not on `PATH`. Every message SHALL
name the source, so a manifest with several sources always says which one failed, and the
messages SHALL be exactly:
...
- **THEN** resolution fails with `source "shared": rev "47f73fc" not found`, because
  SPEC.md admits a tag, a branch, or a full sha and nothing else, and an abbreviation is
  not stable enough to pin
