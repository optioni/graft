...
- [ ] 0.1 Reuse `internal/cli/sync_acceptance_test.go`'s existing harness — `runGraftIn` with an
...
- [ ] 0.2 RED: Write the failing end-to-end test for the headline scenario — a consumer synced at
...
- [ ] 0.3 Confirm it fails because `update` is not a command — `graft: unknown command "update"` —
...
- [ ] 1.1 RED: Write failing unit tests, over in-memory `[]byte` with no filesystem at all, for
...
- [ ] 1.2 RED: Write failing unit tests for the locating half: *A rev key in a kinds sub-table is
...
- [ ] 1.3 RED: Write failing unit tests for the character rule and the round trip: *A rev that
...
- [ ] 1.4 GREEN: Implement `manifest.SetRev(data []byte, name, rev string) ([]byte, error)`.
      ...
      standard table
- [ ] 1.5 GREEN: Inside that table, skip comment lines, match `rev` with optional leading
...
- [ ] 1.6 GREEN: Pin the two error strings design.md → Contracts lists, both prefixed
...
- [ ] 1.7 GREEN: Add `manifest.Read(path) (*Manifest, []byte, error)` and make `Load` delegate to
      it, so `graft.toml not found` keeps its single spelling and no existing caller or test moves
- [ ] 1.8 REFACTOR: Confirm `SetRev` returns `nil` bytes on every error path, so no caller can
      write a half-edited file, and that nothing in the function touches the filesystem
- [ ] 1.9 CHECK: Contract gate — re-read SPEC.md's `graft.toml` section and confirm the shape
...
- [ ] 1.10 Run `go test ./internal/manifest/` — green, and `go test ./internal/sync/` unchanged
...
- [ ] 2.1 RED: Write failing tests, driven by hand-built plans against a real `t.TempDir()`, for:
      *Manifest bytes are written just before the lock*, *An apply with no manifest bytes leaves
      ...
      failed apply*
- [ ] 2.2 GREEN: Add `apply.Option` and `apply.WithManifest(data []byte)`, and make
      `Run(root, trees, p, opts ...Option)` variadic so all 56 existing call sites compile and
      assert unchanged
- [ ] 2.3 GREEN: Write the bytes after the prune set and the empty-directory walk and immediately
      ...
      anything fails
- [ ] 2.4 GREEN: Add `graft.toml` to the pre-flight pass beside the existing
...
- [ ] 2.5 REFACTOR: State in a comment why this write does **not** go through
...
- [ ] 3.1 RED: Extend `internal/sync/fixture_test.go` with an `update` helper beside `run` and
...
- [ ] 3.2 RED: Write the failing prune test — *An item the new rev no longer provides is removed,
...
- [ ] 3.3 RED: Write failing tests for the source argument: *Only the named source is
...
- [ ] 3.4 RED: Write failing tests for the narrowed pin check: *An update repairs a manifest whose
...
- [ ] 3.5 GREEN: Add `sync.Update{Source, To}` and `sync.Options.Update *Update`, with a doc
...
- [ ] 3.6 GREEN: Refuse an `Update.Source` the manifest does not declare with
...
- [ ] 3.7 GREEN: Hand `lock.CheckPins` only the sources this run does not re-resolve, and take a
      re-resolved source's sha from `source.Resolve` rather than from the lock
- [ ] 3.8 REFACTOR: Confirm by reading the function that `plan`, `catalog`, and `apply` see no
      difference between a sync and an update, and that the only branch is which sha a source gets
- [ ] 3.9 Run `go test ./internal/sync/` — green, every pre-existing sync test unchanged

## 4. sync: `--to` moves the pin, and the report says so
<!-- kind: behavior -->

- [ ] 4.1 RED: Write failing tests for: *The pin moves and the rest of the file survives* (a
...
- [ ] 4.2 RED: Write failing tests for the report: *A bumped tag shows both revs and both shas*
      ...
      rendering
- [ ] 4.3 GREEN: When `Update.To` is set, run `manifest.SetRev` on the bytes `manifest.Read`
      returned, and hand the result to `apply.Run` through `WithManifest`
- [ ] 4.4 GREEN: Re-parse the edited bytes and **assert the named source's rev is `Update.To`**
...
- [ ] 4.5 REFACTOR: State in a comment why the edited bytes are re-parsed rather than the parsed
...
- [ ] 4.6 CHECK: Contract gate — this group is what causes `graft.toml` to be rewritten in a real
...
- [ ] 4.7 Run `go test ./internal/sync/` — green
